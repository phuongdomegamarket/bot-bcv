            ...
            async with session.post(
                ...
                try:
                    ...
                    if jsonData and "sessionId" in jsonData:
                        ...
                        return {**jsonData, "browserId": browser_id}
                    print("Login failed")
                except Exception as e:
                    print(f"Error: {e}")
                return None
...
async def getTextFromImage1(url):
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            try:
                image_bytes = await response.read()
                ...
                if response.status < 400:
                    for name, cookie in response.cookies.items():
                        cookies += f"{name}={cookie};"
                csrf_token = re.search(
                    r'"X-CSRF-TOKEN"\s*:\s*"([^"]+)"', responseText
                ).group(1)
                url = f"https://www.imagetotext.info/emd/captcha-verify/{datetime.now().timestamp()}"
                payload = {
                    "emd_captcha_1": "1Ux6aoGeookM8oIstJ5RAy4WV6MMvRSEh1lDKsLfIn",
                    "emd_captcha_2": None,
                    "emd_captcha_3": datetime.now().timestamp(),
                ...
                response = await session.post(url, headers=headers, data=payload)
                jsonData = await response.json()
                url = "https://www.imagetotext.info/free-image-to-text"
                payload = {
                    "req_key": jsonData["req_key"],
                    "base64": data_uri,
                    "imgName": datetime.now(),
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            try:
                ...
                mime_type = response.headers.get("Content-Type", "image/jpeg")
                data_uri = f"data:{mime_type};base64,{base64_str}"
                url = "https://quillbot.com/api/raven/quill-chat/output"
                payload = {
                    "message": {
                        "role": "user",
                        "content": "Extract text from the image.",
                        "files": [
                            {
                                "name": "34a55fa1-1bb1-4f96-a9fa-dfdc1b734ed7.jpg",
                                "mimeType": "image/jpeg",
                                "file_data": data_uri,
                            }
                        ],
...
async def getAccountList(
    ...
    payload = encrypt_request(
        {**default_meta, "accountType": "ALL", "lang": "en", "mid": 8},
        public_key_base64,
        DEFAULTS["server_public_key_base64"],
    )
    async with aiohttp.ClientSession() as session:
